"""
fig2b1.py — Figura 2b del RL-QLS (entrenamiento), semilla 2 (arranque mas alto).
...
SEED = 2                      # <- semilla "normal" (primaria por defecto)
OUTNAME = "fig2b1.png"
...
def plot_2b(history_path, out_path):
    ...
    print(f"[fig2b1] semilla {h.get('seed')}: arranque azul={avg[0]:.2f}, "
          f"convergencia={steps[-100:].mean():.2f}, "
          f"espigas>22={n_clip} ({100*n_clip/N:.1f}%)")
    print(f"[fig2b1] guardada: {out_path}")
